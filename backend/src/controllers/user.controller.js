...
export async function getRecommendedUser(req, res) {
    try {
        ...
        res.status(500).json({ message: "An error occurred while fetching recommended users." });
    }
}
export async function getMyFriends(req, res) {
    try {
        const user = await User.findById(req.user.id).select('friends').populate('friends', "fullName profilePic nativeLanguage learnningLanguage");
...
export async function sendFriendRequest(req, res) {
    try {
        ...
        // prevent sending self friend request
        if (myId === recipientId) {
...
export async function acceptFriendRequest(req, res) {
    try {
        ...
        //$addToSet adds a new element to an array if it doesn't already exist, otherwise it updates the existing element
...
export async function getFriendRequests(req, res) {
    try {
        const incomingReqs = await FriendRequest.find({ recipient: req.user.id, status: "pending" }).populate('sender', "fullName profilePic nativeLanguage learnningLanguage");
...
export async function getOutgoingFriendRequests(req, res) {
    try {
        const outgoingRequests = await FriendRequest.find({ 
            sender: req.user.id,
             status: "pending",
             }).populate('recipient', "fullName profilePic nativeLanguage learnningLanguage");
